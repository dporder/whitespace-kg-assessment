...
The check vocabulary is shared contract, pinned here because two stages independently invented names for the same checks and their reports disagreed over identical trees. Canonical check ids: `child_left_edge`, `own_box_above_first_child`, `siblings_ascend`, `sibling_overlap`, `extent_nests`, `extent_covers_own`, `numbering_gap`, `branch_or_leaf`, `order_preorder`, `path_parent`, `content_hash`, `page_range`, `one_box_per_page`, `citable_flags`, plus two stage 8 only checks with no stage 2 counterpart, `label_nesting` and `ref_span_integrity`. The tolerance map is pinned beside `PARSE_GEOMETRY` in `config.py`: indent tolerance for the left edge and extent nesting comparisons, vertical tolerance for ascent and own-above, and sibling overlap at the larger of vertical tolerance and the overlap share of the smaller box. Own-box checks (`child_left_edge`, `own_box_above_first_child`) compare own ink only: a node with no `bboxes_own` (tables, form rows, parts) has no own edge, is counted not examined with the reason recorded, and its containment is `extent_nests`'s job, not a fallback comparison against its extent. An anomaly explains a violation only when its key starts with the check id, it sits on one of the two nodes the check compared, one anomaly instance explains one violation, and a gap anomaly naming a different follower than observed never matches. Stage 2 therefore stamps explanations as `<check_id>_<reason>` on the violating node, one per violation. Geometry tolerances come from `config.PARSE_GEOMETRY` in both stages, one number, two readers, or the two reports count different violations over the same ink. Stage 2's manifest is `output/<run>/violations.json`, at the run root, never inside `tree/`, which holds only part files.
...
Location is the character offsets into the parent's text, and the ref's box is derivable from those, cached on the node for the review UI. Its `path` is the parent's path plus a span suffix, `/ref@<start>-<end>`, so its id is deterministic like every other node's. Any two refs sharing one span disambiguate with an ordinal, `/ref@<start>-<end>+<n>`: interior members minted by range expansion in expansion order, and any other same-span pair (an LLM-extracted span overlapping a grammar citation, say) in emission order, deduped first when they cite the same target, with a note recorded, a note being an observation and not a violation. Stage 3 writes `output/<run>/refs/<part>.json` as a flat `RefsFile`, `{"part", "refs": [ref nodes]}`, because stage 2 trees carry no ref children, stage 7 attaches each ref to its parent by path. Citations inside a heading's `title` have no text to anchor to and are recorded for triage rather than minted. Tonight a ref's cached boxes are its citing node's own boxes, the whole provision rather than the span's line, an approximation recorded here so the review UI's crop is read accordingly; the line-precise box from character offsets is the refinement path. The document root node materialises at load, stage 7, carrying the document metadata fields; stage 2 trees are per part and emit no root. A phrase citing several targets ("Clauses 2.10, 9, 14, 15, 27") becomes one ref per cited target, each anchored to its own number's characters, sharing a `group_id`. Ranges expand inclusively, the interpretation clause says series are inclusive. An implied interior member of a range ("15.3" in "15.2 to 15.4") has no printed characters of its own, so it anchors to the whole range phrase, the only ink that names it, with the same-span ordinal disambiguating its path, an anomaly recording the implication, and overlap with its printed siblings' anchors expected there rather than a violation. A viewer highlighting an implied member highlights the phrase that implies it, which is the honest answer. Refs never mint target nodes, a citation to something the corpus does not contain stays unresolved with candidates kept. Precedent, Akoma Ntoso models inline citations as elements of the document tree, so there is one schema and resolution state is just the mutable part of a ref node.
...
**Resolution** applies the scope rules, part local first, then the document level rules from Joint Schedule 1 paragraphs 1.3.8 and 1.3.9. Nearness is a tree walk to the nearest enclosing scope, never raw character distance, with proximity kept only as a candidate scoring feature in the ambiguous residue. The nearest enclosing scope is the nearest division the drafters themselves name as a Part before the part itself, mirroring definition scoping: Call-Off Schedule 9's Parts A and B restart paragraph numbering, so "Paragraph 3.4" cited inside Part B means Part B's 3.4, and the first real audit run caught the flat part-local reading resolving 17 such refs across the sub-part boundary, every one with a nearer same-numbered node in its own sub-part. LLM residue calls present the top 5 candidates together and must accept `NONE`, and a `NONE` keeps status unresolved with candidates kept.
...
Definition site: `{"term", "definition_node_id", "source": "declared | discovered | both", "scope": "document | part local, with the part", "aliases": ["CCS"], "pointer": "optional, when the definition delegates, e.g. to Schedule 6"}`. Scope is the PATH of the nearest scoping ancestor, `document`, `part:<part-id>`, or deeper where the ink demands it: Call-Off Schedule 9 defines "Breach of Security" once in Part A and again, differently, in Part B, so those sites carry `part:call-off-schedule-9/<sub-part>` and (term, scope) stays unique while the shadowing follows the sub-part a use stands in. A term genuinely defined twice at one scope would be a drafting defect to record, not a key to overwrite. Aliases are captured from the parenthetical abbreviation convention at first use, the Crown Commercial Service (CCS), and matched with the same rules as full forms. Definition texts are themselves matched for term uses, and every term used inside another term's definition yields a deterministic `DEFINED_USING` edge between the two Term nodes, the vocabulary's own dependency graph. Stage 8 reports its cycles and maximum chain depth as anomalies.
...
- Top level headings shaped `3. Title` (some schedules print the number alone on its line, title beside or below, confirmed by typography), body numbering `3.1`, `3.1.2`, a fourth dotted level `2.1.1.1` in three schedules, lettered `(a)` or bare `a)` and roman `(i)` items below. Dotted numbers may carry a trailing period. Four level dotted numbers and lettered or roman sub paragraphs are both kind `item`, the deepest addressable unit.
...
- Verified counts across all 475 pages: 46 four level dotted numbers, concentrated in Call-Off Schedule 6 (27), Call-Off Schedule 22 (14) and Joint Schedule 8 (5), e.g. `2.1.1.1` on p193 and `9.1.3.2` on p202. They parse as kind `item`, the deepest addressable unit, per the kind by function rule, and `max_dotted_depth` is 4. An earlier claim here of zero four level numbers was wrong, see REVIEW-NOTES. Also 522 lettered items, 82 roman items. Within Core Terms alone, 144 two level, 43 three level, 169 lettered, 1 roman, so do not calibrate the grammar on Core Terms and assume it generalises, roman items are a real level in the schedules. The earlier count of 146 two level provisions included two cross references that start a wrapped line ("...10.4.3, 10.5 or 20.2 or a Contract expires..."), which the parser records as `numbering_read_as_wrapped_text` rather than minting phantom provisions.
