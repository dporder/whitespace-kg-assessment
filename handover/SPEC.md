...
The decisions record is the contract seam between the two surfaces, so its vocabulary is pinned here and detailed in `pipeline/eval/GOLDEN_FORMAT.md`, which elaborates, never contradicts. Ref verdicts: `target` (the pipeline's target or a picked candidate is right, `chosen_candidate` required), `unresolvable` (a real citation with no target in the corpus, the label the abstention gate feeds on, and the UI must make it reachable), `not_a_reference` (not a citation at all). Term verdicts: `use` (`chosen_candidate` names the governing term, which may differ from the matched one in alias collisions) and `not_a_use`. Anomaly verdicts: `confirmed`, `rejected`, plus the triage set `agree`, `parser_wrong`, `outline_wrong`, `both_differ` for derived versus provided rows. Anomaly records carry an `anomaly_index` so two anomalies on one node hold separate verdicts. Demo and test flows write only to temporary paths, so a `decisions.jsonl` in the repo always holds real reviewer verdicts.
